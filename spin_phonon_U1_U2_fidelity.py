...
class U1rwa:
    ...
    def unitary(self, t):
        ...
        a10 = self.alpha_10(t)
        self.uni = self.g * (
            -(a01 * self.h_interaction_term_1) - (a10 * self.h_interaction_term_2)
        )
        return self.uni
    ...
    def generate_interaction_term(self, boson_op="create", spin_op="+"):
        ...
        if boson_op in ["create", "c"]:
            ...
            a = quimb.destroy(self.n_phonons)
        ham = sum(
            [quimb.ikron([a, s], self.dims, [0, i + 1]) for i in range(self.n_ions)]
        )
        return ham
...
class U2rwaXYDyson:
    def __init__(self, n_phonons, n_ions, g, omega_eff, omega_single_mode):
        ...
        self.Delta = omega_eff - omega_single_mode
        self.Sum = omega_eff + omega_single_mode
        self.h_interaction_term = self.generate_interaction_term()
        self.unitary(self.t)
    ...
    def unitary(self, t):
        ...
        coef = (
            ...
            * self.omega_single_mode
            / (2 * ((self.omega_eff ** 2) - (self.omega_single_mode ** 2)))
        )
        self.uni = -1.0j * self.t * coef * self.h_interaction_term
        return self.uni

    def generate_interaction_term(self):
        paulix = 2 * quimb.spin_operator("x", sparse=True)
        pauliy = 2 * quimb.spin_operator("y", sparse=True)
        ham = sum(
            [
                quimb.ikron(
                    [paulix, paulix],
...
if __name__ == "__main__":
    ...
    steps = 1000
    print_step = 10
    times = list(np.linspace(0, 0.001, steps))
    target_alpha = 0.2
    r = 1
    ...
    a = 90 / (np.sqrt(90 ** 2 + 10 ** 2))
    b = 10 / (np.sqrt(90 ** 2 + 10 ** 2))
    psi1 = (a * quimb.kron(init_boson, init_spin_xy)) + (
        b * quimb.kron(single_boson, groundstate)
    )
    ...
    U2_XY_dyson = U2rwaXYDyson(
        n_phonons, n_ions, gs[0] / 2, omega_eff, omega_single_mode
    )
    U2_XY = U2rwaXY(n_phonons, n_ions, gs[0] / 2, omega_eff, omega_single_mode)
    ...
    fidelities_xy = []
    fidelities_xy_dyson = []
    fidelities_full_dyson = []
    count = 0
    for t in times:
        # U = identity + U1.unitary(t) + U2.unitary(t)
        # U = U1.unitary(t) + U2_XY.unitary(t) + U2_bounded.unitary(t)
        ep = gs[0] ** 2 / (2 * (omega_eff - omega_single_mode) ** 2)
        jcm_prefactor = np.sqrt(ep)
        xy_prefactor = np.sqrt(1 - ep)
        # U_xy = U1.unitary(t) + U2_XY.unitary(t) + U2_bounded.unitary(t)
        U_xy = jcm_prefactor * U1.unitary(t) + xy_prefactor * U2_XY.unitary(t)
        # U_xy_dyson = identity + U1.unitary(t) + U2_XY_dyson.unitary(t)
        # U_full = identity + U1.unitary(t) + U2.unitary(t)
        # U_full = identity + U2.unitary(t)
        # U_full = U_full / (trace(U_full.H @ U_full) / trace(identity))
        psi_t_xy = U_xy @ psi0
        # psi_t_xy_dyson = U_xy_dyson @ psi0
        # psi_t_full = U_full @ psi0
        f_xy = c_fidelity(t, psi_t_xy)
        # f_xy_dyson = c_fidelity(t, psi_t_xy_dyson)
        # f_full = c_fidelity(t, psi_t_full)
        fidelities_xy.append(f_xy)
        # fidelities_xy_dyson.append(f_xy_dyson)
        # fidelities_full_dyson.append(f_full)
        count += 1
        if not count % print_step:
            print(
                f"--> Computed unitary for t = {t} ({count/steps*100}%)"
                # f"\n\t - Tr[U(t)] = {trace(U)}"
                # f"\n\t - Tr[U(t).H U(t)] = {trace(U.H @ U)}"
                # f"\n\t - trace(U_full.H @ U_full) / trace(identity) = {trace(U_full.H @ U_full) / trace(identity)}"
                f"\n\t - F(t) of XY = {f_xy}"
                # f"\n\t - F(t) of XY Dyson = {f_xy_dyson}"
                # f"\n\t - F(t) of full = {f_full}"
            )
