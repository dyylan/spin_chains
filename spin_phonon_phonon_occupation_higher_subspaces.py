...
if __name__ == "__main__":
    n_ions = 10
    n_phonons = 4
    times_microseconds = [0, 200]  # [i * 1 for i in range(0, 101, 1)]
    target_alpha = 0.2
    plot_steps = True
    update_data = True
    subspace_n_over = 2
    ...
    for i, time in enumerate(times_microseconds[:-1]):
        ...
        if plot_steps:
            ...
            legend_leakage = []
            if (
                calc_parameter == "leakage"
                or calc_parameter == "phonons"
                or calc_parameter == "n_phonons"
            ):
                ...
                rs = [1.0002584]
                rs = [0.99987, 1, 1.000338]
                rs = [1, 1.000321, 1.00027]
                rs = [1.000261, 1.000262, 1.000263]
                rs = [1.000262]
                for r in rs:
                    Es = [
                    ...
                    legend_leakage.append(f"r = {r}")

                ax.legend(["sim"] + legend_leakage)

            # ax.plot(times, fidelity)

            ax.set_xlabel("Time (s)")
            if calc_parameter == "n_phonons":
                ax.set_ylabel("$\\bar{n}(t)$")
            else:
                ax.set_ylabel(calc_parameter.capitalize())

            if calc_parameter == "phonons":
                ax.legend([f"{i} phonons" for i in range(n_phonons)] + legend_leakage)
            ax.plot()
            plt.show()
