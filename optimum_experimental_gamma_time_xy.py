    ...
    for spins in spins_list:
        if use_existing_mu:
            data_orig = data_handling.read_data_spin(
                "experimental/always_on_fast_xy",
                "open",
                target_alpha,
                use_existing_mu,
                spins,
            ...
            ion_trap = iontrap.IonTrapXY(spins, use_optimal_omega=True)
        ...
        gamma_range = [
            approx_gamma * gamma_range_factor * (1 / gamma_offset_factor),
            (approx_gamma / gamma_range_factor) * (1 / gamma_offset_factor),
        ]
        # gamma_range = [1 / 1190, 1 / 1210]
        (
            optimum_gamma,
            fidelity,
            time,
...
def fidelity_time(
    ...
    dt = 0.1 if gamma_rescale else 2e-7
...
def approximate_gamma(Js):
    H = 4 * Js
    eigenvectors, eigenvalues = hamiltonians.Hamiltonian.spectrum(H)
...
if __name__ == "__main__":
    target_alpha = 0.6
    protocol = "always_on_fast_xy"
    chain = "open"
    mid_n = False
    end_n = True
    n_factor = 0  # 0 turns off n_factor (not 1)
    n_diff = 0
    save_tag = "new_optimum_gammas"
    ...
    save_tag += f"_mu_min"
    always_on = True if protocol in ["always_on_fast", "always_on_fast_xy"] else False
    open_chain = True if chain == "open" else False

    # spins_list = [x for x in range(4, 124, 4)]
    # spins_list = [x for x in range(12, 40, 4)]
    # spins_list = [x for x in range(6, 14, 2)]
    # spins_list = [x for x in range(14, 20, 2)]
    # spins_list = [x for x in range(20, 28, 2)]
    # spins_list = [x for x in range(28, 40, 2)]
    spins_list = [x for x in range(6, 54, 2)]
    spins_list = [x for x in range(26, 54, 2)]
    data = main(
        spins_list,
        target_alpha,
        gamma_range_factor=1.2,
        gamma_offset_factor=1,
        gamma_steps=60,
        open_chain=open_chain,
        always_on=always_on,
        mid_n=mid_n,
