...
if __name__ == "__main__":
    ...
    def calc_init_state_fidelity(t, pt):
        return quimb.expectation(psi0, pt)

    compute = {"time": calc_t, "fidelity": calc_init_state_fidelity}
    ...
    def calc_init_state_fidelity_U1_U2(rho, rho_t):
        return quimb.expectation(rho, rho_t)
    ...
    U2_XY = U2rwaXY(n_phonons, n_ions, Js)
    U2_bounded = U2rwa(n_phonons, n_ions, gs[0] / 2, omega_eff, omega_single_mode)
    rho = psi0 @ psi0.H

    fidelities = []
    count = 0
    for t in times:
        U = identity + U1.unitary(t) + U2_bounded.unitary(t)
        # U = U1.unitary(t) + U2_XY.unitary(t) + U2_bounded.unitary(t)
        # U = U1.unitary(t) + U2_XY.unitary(t)
        # prefactor = 1 - trace(U1.unitary(t) @ rho @ U1.unitary(t).H)
        rho_t = U @ rho @ U.H
        f = calc_init_state_fidelity_U1_U2(rho, rho_t)
        fidelities.append(f)
        count += 1
    ...
    ax.legend(["Full model", "Truncated Dyson series"])
